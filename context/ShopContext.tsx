  ...
  const init = async () => {
      ...
      const safetyTimer = setTimeout(() => {
          ...
          if (isLoading) {
              ...
              setIsLoading(false);
              // Assume offline if connection check hangs to prevent infinite wait states
              setIsOffline(true);
          }
      }, 5000);
  ...
  useEffect(() => {
      ...
      const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
        if (event === 'SIGNED_IN' && session?.user) {
          ...
          // Force Super Admin role for specific email to prevent lockout if metadata is missing
          const role = (session.user.email === 'superadmin@lakkiphones.com')
                         ? 'Super Admin'
